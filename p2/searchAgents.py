...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodGrid.asList():
        food_distances.append(manhattanDistance(position,food))

    return max(food_distances) if len(food_distances) > 0 else 0
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        """
        x,y = state
        distance,closest_food = min([(manhattanDistance(state,food),food) for food in self.food.asList()])
